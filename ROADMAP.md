...
- **Mobile follow-ups (post-pilot)** — only after telemetry from the IXOM rollout shows demand:
  - Offline calibration entry with replay-on-reconnect (IndexedDB queue + conflict resolution UI). Biggest unlock for technicians in plant areas with no signal — but the conflict-resolution UI is non-trivial (two techs editing the same record while offline) and we have no real-world data yet on how often it actually happens. Defer until a pilot site asks.
  - Push notifications for pending approvals (Capacitor Push Notifications + APNs/FCM + per-user opt-in in Settings). Currently approvers get email + an in-app badge; push is nice-to-have, not blocking. Wait until we have feedback that email latency is hurting workflow.
  - iOS App Store submission via Codemagic CI macOS runner — `codemagic.yaml` scaffold landed in the P6 commit. Outstanding: Apple Developer account ($149/yr AUD), App Store Connect app record at bundle ID `com.calcheq.app`, App Store Connect API key uploaded to Codemagic as the "CalCheq App Store Connect" integration, replace `APP_STORE_APPLE_ID` placeholder in `codemagic.yaml`, create "Internal Testers" beta group. Android-first ships first to IXOM.
  - Real device screenshots for the App Store + Play Store listing — currently `mobile/store-metadata/` only has the capture plan. Once we have an Android build on a real device (or the iOS TestFlight build), capture the 6 screenshots called for in that plan and replace the placeholder paths.
