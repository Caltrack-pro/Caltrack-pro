...
function drawPageHeader(doc, title, refNum, issueDate) {
  // Navy background
...
function drawFooter(doc, pageNum, totalPages, isSIS = false) {
  ...
  doc.setTextColor(...SLATE)
  const lines = [
    'This certificate relates only to the item(s) identified herein. Results are valid at the date of calibration stated and shall not be extrapolated.',
    'Calibration results are traceable to the International System of Units (SI) per ISO/IEC 17025:2017 via an unbroken chain of NATA-accredited or equivalent reference standards. Uncertainty at k=2 (≈95% CI).',
    isSIS
      ? 'SIS / Safety Instrument System: This instrument is classified Safety Critical. Records must be retained per IEC 61511-1:2016 / AS 61511.1:2021 functional safety requirements. PROOF TEST interval applies.'
      : 'Prepared in accordance with AS ISO 9001:2016 Quality Management System requirements and internal calibration procedures. This document shall not be reproduced except in full.',
    'This document shall not be reproduced except in full without written approval of Calcheq.',
  ]
  lines.forEach((l, i) => doc.text(l, MARGIN, fy + 4.5 + i * 3.2, { maxWidth: PAGE_W - MARGIN * 2 - 40 }))
  doc.text(
...
export function generateSingleCalibrationCert(instrument, record) {
  const doc      = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' })
  const certNum  = certNumber(record.id, record.calibration_date)
  const issueDate = fmtDate(new Date().toISOString())

  let y = drawPageHeader(doc, 'CALIBRATION CERTIFICATE', certNum, issueDate)
...
export async function generateMultiCalibrationReport(instrument, records) {
  const doc      = new jsPDF({ unit: 'mm', format: 'a4', orientation: 'portrait' })
  ...
  // ── PAGE 1: Summary ───────────────────────────────────────────────────────
  let y = drawPageHeader(doc, 'CALIBRATION HISTORY REPORT', rptNum, issueDate)
  ...
  sorted.forEach((record, idx) => {
    ...
    let ry = drawPageHeader(doc, `CALIBRATION RECORD ${idx + 1} OF ${sorted.length}`, cn, issueDate)
