/**
 * CalTrack Pro — Calibration Report Generator
 *
...
function drawPageHeader(doc, title, refNum, issueDate) {
  ...
  doc.setFontSize(15)
  doc.text('CalTrack Pro', MARGIN, 11)
...
function drawFooter(doc, pageNum, totalPages) {
  ...
  doc.setTextColor(...SLATE)
  const lines = [
    'This certificate relates only to the item(s) identified herein. Results are valid only for the date of calibration stated.',
    'Calibration results are traceable to the International System of Units (SI) via an unbroken chain of calibrated reference standards. Uncertainty reported at k=2 (≈95% confidence).',
    'This document shall not be reproduced except in full without written approval of CalTrack Pro.',
  ]
  lines.forEach((l, i) => doc.text(l, MARGIN, fy + 4 + i * 3.5))
  doc.text(
    `Page ${pageNum} of ${totalPages}  |  Generated by CalTrack Pro on ${fmtDate(new Date().toISOString())}`,
    PAGE_W - MARGIN, fy + 4, { align: 'right' },
  )
}
...
export function generateSingleCalibrationCert(instrument, record) {
  ...
  doc.text(
    'Traceability Statement: All calibration results are traceable to the International System of Units (SI) through an unbroken chain of ' +
    'calibrated reference standards with documented uncertainties. Expanded uncertainty U is reported at a coverage factor k = 2, ' +
    'corresponding to a confidence level of approximately 95%.',
...
export async function generateMultiCalibrationReport(instrument, records) {
  ...
  autoTable(doc, {
    ...
    margin:     { left: MARGIN, right: MARGIN },
    didDrawPage: () => {
      drawFooter(doc, doc.internal.getCurrentPageInfo().pageNumber, sorted.length + 1)
    },
  })
  ...
  for (let p = 1; p <= total2; p++) {
    ...
    doc.setFontSize(6)
    doc.setTextColor(...SLATE)
    doc.text(`Page ${p} of ${total2}  |  Generated by CalTrack Pro on ${fmtDate(new Date().toISOString())}`,
